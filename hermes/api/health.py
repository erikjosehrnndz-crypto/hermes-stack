    ...
    try:
        if session is None or session.closed:
            async with aiohttp.ClientSession() as s:
                async with s.get(f"{litellm_url}/health", headers=headers, timeout=aiohttp.ClientTimeout(total=3)) as resp:
...
async def check_elevenlabs(session: aiohttp.ClientSession | None) -> bool:
    ...
    try:
        if session is None or session.closed:
            async with aiohttp.ClientSession() as s:
                async with s.get("https://api.elevenlabs.io/v1/models", headers=headers, timeout=aiohttp.ClientTimeout(total=4)) as resp:
